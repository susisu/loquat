...
describe(".chainr(term, op, defaultVal)", () => {
  it("should return a parser that parses zero or more terms accepted by `term' and operators"
    + " accepted by `op', and reduces the values to a single value from right to left", () => {
    ...
    // csucc, csucc, csucc, cfail
    {
      ...
      const manyParser = chainr(parsers[0], parsers[1], "xyz");
    ...
    // right to left: csucc, csucc, csucc, csucc, csucc, esucc, efail
    {
      ...
      const parsers = generateParsers(consumed, success, vals, states, errs);
      const manyParser = chainr(parsers[0], parsers[1]);
      expect(manyParser).to.be.a.parser;
      const res = manyParser.run(initState);
      expect(Result.equal(
        res,
