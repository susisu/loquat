...
describe(".equal", () => {
  ...
  it("should return false if two positions are different", () => {
    ...
    {
      const posA = new SourcePos("main", 496, 6, 28);
      const posB = new SourcePos("lib", 496, 6, 28);
    ...
    {
      const posA = new SourcePos("main", 496, 6, 28);
      const posB = new SourcePos("main", 497, 6, 28);
      expect(SourcePos.equal(posA, posB)).to.be.false;
      expect(SourcePos.equal(posB, posA)).to.be.false;
    }
    // different lines
    {
      const posA = new SourcePos("main", 496, 6, 28);
      const posB = new SourcePos("main", 496, 7, 28);
      expect(SourcePos.equal(posA, posB)).to.be.false;
      expect(SourcePos.equal(posB, posA)).to.be.false;
    }
    // different columns
    {
      const posA = new SourcePos("main", 496, 6, 28);
      const posB = new SourcePos("main", 496, 6, 29);
      expect(SourcePos.equal(posA, posB)).to.be.false;
      expect(SourcePos.equal(posB, posA)).to.be.false;
    }
    // all different
